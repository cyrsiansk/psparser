...
async def main():
    ...
    async with HTTPClient(opts=opts) as http:
        client = PartySlateClient(http=http,
                                  config=PartySlateClientConfig(default_location="miami",
                                                                default_category="event-vendors",
                                                                concurrency=10))
        vendors = await client.collect_vendors(n=50, start_page=1, fetch_additional_for_each=True)
